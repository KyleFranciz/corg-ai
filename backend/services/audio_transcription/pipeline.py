...
import numpy as np  # might remove
...
# import functions for dependencies
...
# function to handle the silence when talking to the agent
def record_audio_until_silent(
    silence_seconds: float = 1.5, max_seconds: int = 15
) -> np.ndarray:
    np = _import_numpy()
...
def speak_response(
    text: str, model_path: str = PIPER_MODEL_PATH, save_path: str | None = None
) -> None:
    np = _import_numpy()
    sd = _import_sounddevice()
